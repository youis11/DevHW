...
void j1App::FinishUpdate()
{
	...
	sprintf_s(title, 256, "Av.FPS: %.2f Last Frame Ms: %02u Last sec frames: %i  Time since startup: %.3f Frame Count: %lu  Framerate Cap: %i",
			  avg_fps, last_frame_ms, frames_on_last_update, seconds_since_startup, frame_count, framerate_cap);
	...
	if (last_frame_ms < framerate)
	{
		...
		j1PerfTimer t;
		waiting_time = framerate - last_frame_ms;
		SDL_Delay(waiting_time);

	}
	LOG("We waited for %i milliseconds and we got back in %f", t, waiting_time);
